  ...
  const initPay = (order) => {
    ...
    const options = {
      ...
      handler: async (response) => {
        ...
        //   appointmentId // Pass the appointment ID here
        }
      }
    ...
    const appointmentRazorpay = async (appointmentId) => {
      try {
        // Ensure the amount is included in the request to your backend
        // const paymentData = {
        //     amount: 5000, // Make sure this is passed correctly
        //     appointmentId: appointmentId, // Ensure appointmentId is included
        //     currency: "INR" // Assuming INR currency, change if needed
        // };
        ...
        if (data.success) {
          ...
          //     amount: amount.toString(), // Razorpay expects the amount as a string
          //     currency: currency,
          //     name: "Deco Haven",
              ...
              <div key={index} className="bg-white shadow-md rounded-lg p-4 flex flex-col">
                ...
                <div className="mt-auto flex justify-between">
                  {item.cancelled ? (
                    ...
                    <>
                      <button onClick={() => cancelAppointment(item._id)} className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition duration-300">
                        Cancel Appointment
                      </button>
                      <button onClick={() => appointmentRazorpay(item._id)} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition duration-300">
                        Pay Fee Online
                      </button>
                    </>
                  )}
